...
create_vi_meteo_dfs <- function(write_dir) {
    ...
    pck <- (c("tidyr", "rgdal", "ggplot2", "raster",
          ...
          "tableone", "pander", "arsenal", "finalfit", "ggpubr"))
    ...
    read_dir <- write_dir # here("data", "Augmented_data", "Playground_dir_11", "output")
    ...
    write.csv(df_mfc2_four_yrs,
            here(write_dir, "df_mfc2_with_Season.csv"), 
            row.names = FALSE)

    write.csv(df_gor_four_yrs,
            here(write_dir, "df_gor1_with_Season.csv"), 
            row.names = FALSE)
