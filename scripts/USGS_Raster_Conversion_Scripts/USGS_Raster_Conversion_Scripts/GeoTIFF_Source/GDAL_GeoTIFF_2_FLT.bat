...
REM //
REM // 2^) Run GDAL_GeoTIFF_2_FLT.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_GeoTIFF_2_FLT.bat
ECHO #===========================================================================

REM initialize paramters to blanks
SET GDAL_GeoTIFF_2_FLT_SRC=
SET GDAL_GeoTIFF_2_FLT_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
    ...
    REM Source exists so set the GDAL_GeoTIFF_2_FLT_SRC
	SET GDAL_GeoTIFF_2_FLT_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_GeoTIFF_2_FLT_OUTPUT
	  IF NOT EXIST GridFloat_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR GridFloat_Output
	  ) ELSE (
	    IF EXIST GridFloat_Output\GDAL_GeoTIFF_2_FLT.flt (
			ECHO   ERROR^: The specified [Output GridFloat File Name] ^(GridFloat_Output\GDAL_GeoTIFF_2_FLT.flt^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_GeoTIFF_2_FLT_OUTPUT=GridFloat_Output\GDAL_GeoTIFF_2_FLT.flt
	) ELSE (
	  REM Attempt to use parameter 2 for the output
	  IF EXIST "%2" (
	    ECHO   ERROR^: The specified [Output GridFloat File Name] ^(%2^) already exists^!
		ECHO.
		GOTO:BAT_DIRECTIONS
	  ) ELSE (
	    SET GDAL_GeoTIFF_2_FLT_OUTPUT=%2
	  )
	)
  )
)
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -ot Float32 -of Ehdr -a_nodata -9999 %GDAL_GeoTIFF_2_FLT_SRC% %GDAL_GeoTIFF_2_FLT_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -ot Float32 -of Ehdr -a_nodata -9999 %GDAL_GeoTIFF_2_FLT_SRC% %GDAL_GeoTIFF_2_FLT_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -ot Float32 -of Ehdr -a_nodata -9999 %GDAL_GeoTIFF_2_FLT_SRC% %GDAL_GeoTIFF_2_FLT_OUTPUT%
  CALL "gdal_translate.exe" -ot Float32 -of Ehdr -a_nodata -9999 %GDAL_GeoTIFF_2_FLT_SRC% %GDAL_GeoTIFF_2_FLT_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_GeoTIFF_2_FLT ^<GeoTIFF Input File^> [Output GridFloat File Name]
ECHO       ^<GeoTIFF Input File^> = This is the relative or full path to the 
ECHO                                .tif file that contains the GeoTIFF 
ECHO                                data.
ECHO       [Output GridFloat File Name] = This is the relative or full path to the 
ECHO                                GridFloat output file that will be created.  If 
ECHO                                this is not set, it will attempt to make 
ECHO                                a sub-folder under the current directory 
ECHO                                called GridFLT_Output, and a file in that 
ECHO                                folder called GDAL_GeoTIFF_2_FLT.flt.

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_GeoTIFF_2_FLT.bat
ECHO #===========================================================================
