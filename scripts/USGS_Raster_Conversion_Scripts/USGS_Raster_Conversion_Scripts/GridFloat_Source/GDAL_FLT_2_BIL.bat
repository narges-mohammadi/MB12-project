@ECHO OFF
REM // ***************************************************************************
REM // *  GDAL_FLT_2_BIL.bat
REM // *  Use GDAL to convert GridFloat Downloads to BIL Format
REM // *
...
REM //
REM // 2^) Run GDAL_FLT_2_BIL.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_FLT_2_BIL.bat
ECHO #===========================================================================

REM initialize paramters to blanks
SET GDAL_FLT_2_BIL_SRC=
SET GDAL_FLT_2_BIL_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
    ...
    REM Source exists so set the GDAL_FLT_2_BIL_SRC
	SET GDAL_FLT_2_BIL_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_FLT_2_BIL_OUTPUT
	  IF NOT EXIST BIL_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR BIL_Output
	  ) ELSE (
	    IF EXIST BIL_Output\GDAL_FLT_2_BIL.bil (
			ECHO   ERROR^: The specified [Output BIL File Name] ^(BIL_Output\GDAL_FLT_2_BIL.bil^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_FLT_2_BIL_OUTPUT=BIL_Output\GDAL_FLT_2_BIL.bil
	) ELSE (
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -ot Float32 -of EHdr %GDAL_FLT_2_BIL_SRC% %GDAL_FLT_2_BIL_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -ot Float32 -of EHdr %GDAL_FLT_2_BIL_SRC% %GDAL_FLT_2_BIL_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -ot Float32 -of EHdr %GDAL_FLT_2_BIL_SRC% %GDAL_FLT_2_BIL_OUTPUT%
  CALL "gdal_translate.exe" -ot Float32 -of EHdr %GDAL_FLT_2_BIL_SRC% %GDAL_FLT_2_BIL_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_FLT_2_BIL ^<GridFloat Input File^> [Output BIL File Name]
...
ECHO                                folder called GDAL_FLT_2_BIL.bil.

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_FLT_2_BIL.bat
ECHO #===========================================================================
