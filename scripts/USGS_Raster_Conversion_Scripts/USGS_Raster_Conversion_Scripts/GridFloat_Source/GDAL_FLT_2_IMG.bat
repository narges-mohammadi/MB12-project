@ECHO OFF
REM // ***************************************************************************
REM // *  GDAL_FLT_2_IMG.bat
REM // *  Use GDAL to convert GridFloat Downloads to IMG Format
REM // *
...
REM //
REM // 2^) Run GDAL_FLT_2_IMG.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_FLT_2_IMG.bat
ECHO #===========================================================================

REM initialize paramters to blanks
SET GDAL_FLT_2_IMG_SRC=
SET GDAL_FLT_2_IMG_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
    ...
    REM Source exists so set the GDAL_FLT_2_IMG_SRC
	SET GDAL_FLT_2_IMG_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_FLT_2_IMG_OUTPUT
	  IF NOT EXIST IMG_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR IMG_Output
	  ) ELSE (
	    IF EXIST IMG_Output\GDAL_FLT_2_IMG.img (
			ECHO   ERROR^: The specified [Output IMG File Name] ^(IMG_Output\GDAL_FLT_2_IMG.img^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_FLT_2_IMG_OUTPUT=IMG_Output\GDAL_FLT_2_IMG.img
	) ELSE (
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESSED=YES -of HFA %GDAL_FLT_2_IMG_SRC% %GDAL_FLT_2_IMG_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESSED=YES -of HFA %GDAL_FLT_2_IMG_SRC% %GDAL_FLT_2_IMG_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -co COMPRESSED=YES -of HFA %GDAL_FLT_2_IMG_SRC% %GDAL_FLT_2_IMG_OUTPUT%
  CALL "gdal_translate.exe" -co COMPRESSED=YES -of HFA %GDAL_FLT_2_IMG_SRC% %GDAL_FLT_2_IMG_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_FLT_2_IMG ^<GridFloat Input File^> [Output IMG File Name]
...
ECHO                                folder called GDAL_FLT_2_IMG.img.

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_FLT_2_IMG.bat
ECHO #===========================================================================
