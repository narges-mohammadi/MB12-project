@ECHO OFF
REM // ***************************************************************************
REM // *  ArcGIS_Convert_All_FLT_2_IMG.bat
REM // *  Recursively Scans directory for FLT files and runs ArcGIS_FLT_2_ANY on all
REM // *  The IMG will be put in the same directory as the FLT file
REM // *
...
REM //
REM // 1^) Run ArcGIS_Convert_All_FLT_2_IMG.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START ArcGIS_Convert_All_FLT_2_IMG.bat
ECHO #===========================================================================
...
IF /I "%1" EQU "" (
  ...
  ECHO   ERROR^: Missing the parameter for ^<Dir of GridFloat Files^>^!
  ECHO.
  GOTO:BAT_DIRECTIONS
)

REM recursively search for all GridFloat files in the input directory
FOR /R %1 %%A IN (*.flt) DO (
  IF NOT EXIST %%~dA%%~pAArcGIS_FLT_2_ANY.img (
    REM This directory has not been processed, so create the output
	ECHO CALL ArcGIS_FLT_2_ANY.bat %%~dA%%~pA. %%~dA%%~pAArcGIS_FLT_2_ANY.img
	CALL ArcGIS_FLT_2_ANY.bat %%~dA%%~pA. %%~dA%%~pAArcGIS_FLT_2_ANY.img
  )
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  ArcGIS_Convert_All_FLT_2_IMG ^<Dir of GridFloat Files^>
ECHO       ^<Dir of GridFloat Files^> = This is the relative or full path to the 
ECHO                                directory of the GridFloat files that contain 
ECHO                                the GridFloat data.  It will scan
ECHO                                sub-folders recursively and produce IMG
ECHO                                files in the same folder as the FLT files.
pause

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END ArcGIS_Convert_All_FLT_2_IMG.bat
ECHO #===========================================================================
