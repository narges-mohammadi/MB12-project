@ECHO OFF
REM // ***************************************************************************
REM // *  GDAL_JP2_2_tif.bat
REM // *  Use GDAL to convert JP2 Downloads to tif Format
REM // *
...
REM //
REM // 2^) Run GDAL_JP2_2_tif.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_JP2_2_GeoTIFF.bat
ECHO #===========================================================================

REM initialize parameters to blanks
SET GDAL_JP2_2_GeoTIFF_SRC=
SET GDAL_JP2_2_GeoTIFF_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
    ...
    REM Source exists so set the GDAL_JP2_2_GeoTIFF_SRC
	SET GDAL_JP2_2_GeoTIFF_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_JP2_2_GeoTIFF_OUTPUT
	  IF NOT EXIST tif_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR tif_Output
	  ) ELSE (
	    IF EXIST tif_Output\GDAL_JP2_2_tif.tif (
			ECHO   ERROR^: The specified [Output tif File Name] ^(tif_Output\GDAL_JP2_2_tif.tif^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_JP2_2_GeoTIFF_OUTPUT=tif_Output\GDAL_JP2_2_tif.tif
	) ELSE (
	  REM Attempt to use parameter 2 for the output
	  IF EXIST "%2" (
	    ECHO   ERROR^: The specified [Output tif File Name] ^(%2^) already exists^!
		ECHO.
		GOTO:BAT_DIRECTIONS
	  ) ELSE (
	    SET GDAL_JP2_2_GeoTIFF_OUTPUT=%2
	  )
	)
  )
)
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -of GTiff %GDAL_JP2_2_GeoTIFF_SRC% %GDAL_JP2_2_GeoTIFF_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -of GTiff %GDAL_JP2_2_GeoTIFF_SRC% %GDAL_JP2_2_GeoTIFF_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -of GTiff %GDAL_JP2_2_GeoTIFF_SRC% %GDAL_JP2_2_GeoTIFF_OUTPUT%
  CALL "gdal_translate.exe" -of GTiff %GDAL_JP2_2_GeoTIFF_SRC% %GDAL_JP2_2_GeoTIFF_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_JP2_2_GeoTIFF ^<JP2 Input File^> [Output tif File Name]
ECHO       ^<JP2 Input File^> = This is the relative or full path to the 
ECHO                                JP2 file that contains the JP2 
ECHO                                data.
ECHO       [Output tif File Name] = This is the relative or full path to the 
ECHO                                tif output file that will be created.  If 
ECHO                                this is not set, it will attempt to make 
ECHO                                a sub-folder under the current directory 
ECHO                                called tif_Output, and a file in that 
ECHO                                folder called GDAL_JP2_2_tif.tif.
pause

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_JP2_2_GeoTIFF.bat
ECHO #===========================================================================
