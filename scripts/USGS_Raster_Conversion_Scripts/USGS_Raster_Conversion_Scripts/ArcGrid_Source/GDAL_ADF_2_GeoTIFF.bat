@ECHO OFF
REM // ***************************************************************************
REM // *  GDAL_ADF_2_GeoTIFF.bat
REM // *  Use GDAL to convert ArcGrid Downloads to GeoTIFF Format
REM // *
...
REM //
REM // 2^) Run GDAL_ADF_2_GeoTIFF.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_ADF_2_GeoTIFF.bat
ECHO #===========================================================================

REM initialize paramters to blanks
SET GDAL_ADF_2_GeoTIFF_SRC=
SET GDAL_ADF_2_GeoTIFF_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
    ...
    REM Source exists so set the GDAL_ADF_2_GeoTIFF_SRC
	SET GDAL_ADF_2_GeoTIFF_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_ADF_2_GeoTIFF_OUTPUT
	  IF NOT EXIST GeoTIFF_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR GeoTIFF_Output
	  ) ELSE (
	    IF EXIST GeoTIFF_Output\GDAL_ADF_2_GeoTIFF.tif (
			ECHO   ERROR^: The specified [Output GeoTIFF File Name] ^(GeoTIFF_Output\GDAL_ADF_2_GeoTIFF.tif^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_ADF_2_GeoTIFF_OUTPUT=GeoTIFF_Output\GDAL_ADF_2_GeoTIFF.tif
	) ELSE (
	  ...
	  IF EXIST "%2" (
	    ...
	    SET GDAL_ADF_2_GeoTIFF_OUTPUT=%2
	  )
	)
  )
)
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_ADF_2_GeoTIFF_SRC% %GDAL_ADF_2_GeoTIFF_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_ADF_2_GeoTIFF_SRC% %GDAL_ADF_2_GeoTIFF_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_ADF_2_GeoTIFF_SRC% %GDAL_ADF_2_GeoTIFF_OUTPUT%
  CALL "gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_ADF_2_GeoTIFF_SRC% %GDAL_ADF_2_GeoTIFF_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_ADF_2_GeoTIFF ^<ArcGrid Input Dir^> [Output GeoTIFF File Name]
ECHO       ^<ArcGrid Input Dir^> = This is the relative or full path to the 
ECHO                                ArcGrid directory that contains the ArcGrid 
ECHO                                data.
...
ECHO                                folder called GDAL_ADF_2_GeoTIFF.tif.
pause

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_ADF_2_GeoTIFF.bat
ECHO #===========================================================================
