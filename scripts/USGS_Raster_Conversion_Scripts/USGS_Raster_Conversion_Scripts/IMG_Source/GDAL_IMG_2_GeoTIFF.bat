@ECHO OFF
REM // ***************************************************************************
REM // *  GDAL_IMG_2_GeoTIFF.bat
REM // *  Use GDAL to convert GeoTIFF Downloads to IMG Format
REM // *
...
REM //
REM // 2^) Run GDAL_IMG_2_GeoTIFF.bat to obtain USAGE directions
REM //
REM //*****************************************************************************

ECHO #===========================================================================
ECHO START GDAL_IMG_2_GeoTIFF.bat
ECHO #===========================================================================

REM initialize paramters to blanks
SET GDAL_IMG_2_GeoTIFF_SRC=
SET GDAL_IMG_2_GeoTIFF_OUTPUT=
...
IF /I "%1" EQU "" (
  ...
  IF NOT EXIST "%1" (
	REM IMG SRC does not exist, so print out the USAGE
	ECHO   ERROR^: ^<IMG Input File^> ^(%1^) does not exist^!
	ECHO.
	GOTO:BAT_DIRECTIONS
  ) ELSE (
    REM Source exists so set the GDAL_IMG_2_GeoTIFF_SRC
	SET GDAL_IMG_2_GeoTIFF_SRC=%1
	
    REM Now check for the output location
    IF /I "%2" EQU "" (
	  REM The output location was not specified, so attempt to make local output directory and set GDAL_IMG_2_GeoTIFF_OUTPUT
	  IF NOT EXIST IMG_Output\NUL (
	    REM Output Directory does not exist, so create it.
		MKDIR IMG_Output
	  ) ELSE (
	    IF EXIST IMG_Output\GDAL_IMG_2_GeoTIFF.tif (
			ECHO   ERROR^: The specified [Output IMG File Name] ^(IMG_Output\GDAL_IMG_2_GeoTIFF.tif^) already exists^!
			ECHO.
			GOTO:BAT_DIRECTIONS
		)
	  )
      SET GDAL_IMG_2_GeoTIFF_OUTPUT=IMG_Output\GDAL_IMG_2_GeoTIFF.tif
	) ELSE (
	  ...
	  IF EXIST "%2" (
	    ...
	    SET GDAL_IMG_2_GeoTIFF_OUTPUT=%2
	  )
	)
  )
)
...
IF EXIST "c:\Program Files\GDAL\gdal_translate.exe" (
  ECHO "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_IMG_2_GeoTIFF_SRC% %GDAL_IMG_2_GeoTIFF_OUTPUT%
  CALL "c:\Program Files\GDAL\gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_IMG_2_GeoTIFF_SRC% %GDAL_IMG_2_GeoTIFF_OUTPUT%
) ELSE (
  ECHO "gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_IMG_2_GeoTIFF_SRC% %GDAL_IMG_2_GeoTIFF_OUTPUT%
  CALL "gdal_translate.exe" -co COMPRESS=LZW -co PREDICTOR=3 -co TFW=YES -of GTiff %GDAL_IMG_2_GeoTIFF_SRC% %GDAL_IMG_2_GeoTIFF_OUTPUT%
)
...
:BAT_DIRECTIONS
ECHO   USAGE^:  GDAL_IMG_2_GeoTIFF ^<IMG Input File^> [Output GeoTIFF File Name]
ECHO       ^<IMG Input File^> = This is the relative or full path to the 
ECHO                                IMG file that contains the source 
ECHO                                data.
...
ECHO                                folder called GDAL_IMG_2_GeoTIFF.tif.
pause

REM finish the script...
:End_BAT
ECHO #===========================================================================
ECHO END GDAL_IMG_2_GeoTIFF.bat
ECHO #===========================================================================
